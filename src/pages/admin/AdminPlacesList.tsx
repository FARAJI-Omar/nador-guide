...
/**
 * Admin Places List - Modern & Minimalist Design
 * Redux integrated for state management
 * 
 * Features:
 * - Show all places (active + inactive)
...
const AdminPlacesList = () => {
  ...
  const filteredAndSortedPlaces = useMemo(() => {
    return places
      .filter((place: Place) => {
        if (filterCategory !== 'all' && place.category.slug !== filterCategory) {
          return false;
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 mb-2">Manage Places</h2>
              ...
              {filteredAndSortedPlaces.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-16 text-center">
                    ...
                    <p className="text-slate-500">No places found matching your filters.</p>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={place.id} className="hover:bg-slate-50 transition-colors duration-150">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <img
                          src={place.images[0] || 'https://via.placeholder.com/50'}
                          alt={place.name}
                    ...
                    <td className="px-6 py-4 text-slate-700 font-medium">{place.price || '-'}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        >
                          {place.isActive ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
